#!/bin/bash
set -eo pipefail
stack_name=flink-sliding-window-app
application_name=flink-sliding-window-app
...
echo "Building code"
mvn -q -f pom-sliding-window.xml clean package -DskipTests
echo "Copying jar"
aws s3 cp target/amazon-msf-slidingwindow-app-1.0.jar s3://aksh-code-binaries/flink/
...
aws cloudformation deploy --template-file ${template_file} --capabilities CAPABILITY_NAMED_IAM CAPABILITY_AUTO_EXPAND \
     ...
     CodeKey=flink/amazon-msf-slidingwindow-app-1.0.jar \
     KafkaBootstrapserver=${bootstrap_server} \
     Parallelism=3 \
     KafkaSinkTopic=windowing-sliding-sink \
